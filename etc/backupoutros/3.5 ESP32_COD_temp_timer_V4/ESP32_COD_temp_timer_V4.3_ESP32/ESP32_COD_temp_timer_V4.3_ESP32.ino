...
const int sda_pin = 23; // definição do pino I2C SDA
...
unsigned long periodo_json = 3600000; // 1 hr = 3600000
...
// Genericos
JsonObject& cod       = object.createNestedObject("codigo");

JsonObject& dt_hr_ini = object.createNestedObject("col_dt_hr_ini");
JsonObject& dt_hr_fim = object.createNestedObject("col_dt_hr_fim");

//Atributos para BLE, BPM
JsonObject& bpm = object.createNestedObject("cardiaco");
...
JsonObject& umi  = object.createNestedObject("umid");

// Atributos para o LUMI
JsonObject& tempo_claro  = object.createNestedObject("luz_c");
JsonObject& tempo_sombra  = object.createNestedObject("luz_s");
JsonObject& tempo_escuro  = object.createNestedObject("luz_e");

//Atributos para o MPU
JsonObject& tempo_empe  = object.createNestedObject("pos_empe");
JsonObject& tempo_deitado  = object.createNestedObject("pos_deita");
JsonObject& tempo_sentado  = object.createNestedObject("pos_sent");
...
String monta_JSON(String tipo){
  if (tipo.equals("X")) {
  object["codigo"]         = "C001";
  object["col_dt_hr_ini"]   = day+barra+month+barra+year+espaco+hour+doispontos+minute+doispontos+second;
  object["col_dt_hr_fim"]   = day+barra+month+barra+year+espaco+hour+doispontos+(minute+10)+doispontos+second;
  object["passos"] = pas;
  object["calorias"] = pas*cal;
  object["cardiaco"] = bpmx;
  //object["temp_interna"]  = temp_inte;
  object["luz_c"]  = min_claro;
  object["luz_s"] = min_sombra;
  object["luz_e"] = min_escuro;
  object["temp"]      = tvar; //1 coleta
  object["umid"]       = hvar; // 1 coleta
  object["pos_sent"] = min_sentado;
  object["pos_deita"] = min_deitado;
  object["pos_empe"]   = min_empe;   
...
bool checaenvio(){
 if(millis() >  time_json + periodo_json){
    ...
    String v = monta_JSON("X");
    Serial.println(v);
    int rsDHT = enviardados(v); //banco de dados
    return true;       
 }
 return false;
}
